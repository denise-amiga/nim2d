...
proc dispatch*(nim2d: Nim2d, evt: SDL_Event) =
  ...
  if t == uint32(SDL_EVENT_QUIT):
    ...
    nim2d.mousemove(
      nim2d,
      evt.motion.x.float * d,
      evt.motion.y.float * d,
      evt.motion.xrel.float * d,
      evt.motion.yrel.float * d,
    )
  elif t == uint32(SDL_EVENT_MOUSE_BUTTON_DOWN):
    let d = SDL_GetWindowPixelDensity(nim2d.gpu.window).float
    nim2d.mousepressed(
      nim2d,
      evt.button.x.float * d,
      evt.button.y.float * d,
      toMouseButton(evt.button.button),
      evt.button.clicks,
    )
    ...
    nim2d.gamepadpressed(
      nim2d, evt.gbutton.which, toGamepadButton(SDL_GamepadButton(evt.gbutton.button))
    )
  elif t == uint32(SDL_EVENT_GAMEPAD_BUTTON_UP):
    nim2d.gamepadreleased(
      nim2d, evt.gbutton.which, toGamepadButton(SDL_GamepadButton(evt.gbutton.button))
    )
    ...
    nim2d.touchpressed(
      nim2d,
      cast[int64](evt.tfinger.fingerID),
      evt.tfinger.x.float * nim2d.width.float,
      evt.tfinger.y.float * nim2d.height.float,
  ...
  elif t == uint32(SDL_EVENT_WINDOW_SHOWN):
    nim2d.window_shown(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_HIDDEN):
    nim2d.window_hidden(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_MOVED):
    nim2d.window_moved(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_RESIZED):
    nim2d.window_resized(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_MINIMIZED):
    nim2d.window_minimized(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_MAXIMIZED):
    nim2d.window_maximized(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_RESTORED):
    nim2d.window_restored(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_MOUSE_ENTER):
    nim2d.window_enter(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_MOUSE_LEAVE):
    nim2d.window_leave(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_FOCUS_GAINED):
    nim2d.window_focus_gained(nim2d)
  elif t == uint32(SDL_EVENT_WINDOW_FOCUS_LOST):
