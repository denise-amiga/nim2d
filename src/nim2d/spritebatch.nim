...
proc addRegion(
  ...
  let c = (
    batch.color.r.float32 / 255,
    batch.color.g.float32 / 255,
    batch.color.b.float32 / 255,
    batch.color.a.float32 / 255,
  )
  ...
  for i in [0'u32, 1, 2, 0, 2, 3]:
    batch.indices.add base + i

proc add*(
    batch: SpriteBatch, x, y: float, angle = 0.0, sx = 1.0, sy = 1.0, ox = 0.0, oy = 0.0
) =
  ## Add one copy of the texture at (x, y), with the same rotation, scale and
  ## origin arguments as drawing an image.
  batch.addRegion(
    batch.texture.width.float, batch.texture.height.float, x, y, angle, sx, sy, ox, oy,
    0, 0, 1, 1,
  )
...
proc add*(
  ...
  batch.addRegion(
    quad.w.float, quad.h.float, x, y, angle, sx, sy, ox, oy, quad.u0, quad.v0, quad.u1,
    quad.v1,
  )
...
proc draw*(batch: SpriteBatch, nim2d: Nim2d, x = 0.0, y = 0.0) =
  ...
  nim2d.gpu.addGeometry(
    pkTextured, nim2d.blend, batch.texture.tex, batch.verts, batch.indices
  )
  nim2d.pop()
