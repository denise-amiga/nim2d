...
let n2d =
  newNim2d("nim2d - touch", 120, 80, W.cint, H.cint, (14'u8, 16'u8, 24'u8, 255'u8))
...
proc colorFor(id: int64): Color =
  ...
  (
    uint8(80 + (h * 7) mod 175),
    uint8(80 + (h * 13) mod 175),
    uint8(80 + (h * 29) mod 175),
    255'u8,
  )
...
n2d.draw = proc(nim2d: Nim2d) =
  ...
  for r in ripples:
    let f = 1.0 - r.t / 1.1
    nim2d.setColor(
      uint8(r.color.r.float * f), uint8(r.color.g.float * f), uint8(r.color.b.float * f)
    )
