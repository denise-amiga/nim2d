...
let n2d =
  newNim2d("nim2d - system", 100, 80, W.cint, H.cint, (16'u8, 18'u8, 26'u8, 255'u8))
...
n2d.keydown = proc(nim2d: Nim2d, sc: Key) =
  ...
  of Key.m:
    nim2d.minimize()
    status = "minimized (restore from the dock)"
  of Key.x:
    nim2d.maximize()
    status = "maximized"
  of Key.z:
    ...
    status = "restored"
  of Key.one:
    nim2d.setSize(640, 480)
    status = "resized to 640x480"
  of Key.two:
    nim2d.setSize(760, 560)
    status = "resized to 760x560"
  of Key.three:
    nim2d.setSize(1024, 640)
    status = "resized to 1024x640"
  of Key.c:
...
n2d.draw = proc(nim2d: Nim2d) =
  ...
  kv(
    160,
    "power",
    power.state & "  " & (if power.percent >= 0: $power.percent & "%"
    else: "n/a"),
  )
  ...
  kv(
    312,
    "dpi/vsync/fps",
    formatFloat(nim2d.getDPIScale, ffDecimal, 2) & "  " &
  ...
  nim2d.print(
    "F fullscreen   Y vsync   R relative   G grab   H cursor   W warp   B msgbox", 30,
    514,
  )
  nim2d.print(
    "M/X/Z minimize maximize restore   1/2/3 resize   C copy   V paste   U url", 30, 532
  )
