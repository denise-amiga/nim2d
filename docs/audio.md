...
`setVolume` sets a source's loudness from 0 to 1, and `setPitch` changes its pitch, which also changes its speed since the two move together, the same as in love. `setLooping` decides whether it repeats.
