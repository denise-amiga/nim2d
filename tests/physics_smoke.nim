...
for i in 0 ..< 300:
  w.update(1.0 / 60.0)
doAssert abs(box.position.y - 9.0) < 0.1,
  "box did not rest on the floor: y = " & $box.position.y
...
block:
  ...
  for i in 0 ..< 240:
    jw.update(1.0 / 60.0)
  let bp = bob.position
  doAssert abs(hypot(bp.x, bp.y) - 3.0) < 0.3,
    "distance joint did not hold: " & $hypot(bp.x, bp.y)
  jw.destroy()
